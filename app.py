# ==============================================================================
# SSS G-ABAY v8.0 - BRANCH OPERATING SYSTEM (FINAL CORRECTED)
...
# ==========================================
st.set_page_config(page_title="SSS G-ABAY v8.0", page_icon="🇵🇭", layout="wide", initial_sidebar_state="collapsed")

# --- TRUE GLOBAL STATE MANAGER (Singleton) ---
@st.cache_resource
class SystemState:
    def __init__(self):
        self.tickets = []
...
# --- INDUSTRIAL CSS (FORCED SIZING & COLORS) ---
st.markdown("""
<style>
    /* Global Cleanups */
    ...
    /* --- CARD BUTTONS (Custom HTML-like styling) --- */
    div.stButton > button {
        box-shadow: 0 4px 10px rgba(0,0,0,0.2);
        transition: transform 0.1s;
    }
    
    /* REGULAR LANE - HUGE BLUE CARD */
    .reg-card > button {
        ...
        border: 6px solid #1E40AF !important;
        text-transform: uppercase;
        display: flex; flex-direction: column; justify-content: center; align-items: center;
    }
    
    /* PRIORITY LANE - HUGE GOLD CARD */
    .prio-card > button {
    ...
    .watermark {
        ...
        color: rgba(0,0,0,0.3); font-family: monospace; font-weight: bold;
...
<div class="watermark no-print">© 2026 rpt/sssgingoog | v8.0</div>
...
def generate_ticket(service, lane_code, is_priority, is_appt=False, appt_name=None):
    ...
    # Determine count for this specific lane prefix
...
# --- MODULE A: THE KIOSK (Fixed Visuals) ---
def render_kiosk():
    # Header
    ...
    # PAGE 5: TICKET DISPLAY (With Cancel & Print)
        ...
        border = "#B45309" if t['type'] == 'PRIORITY' else "#1E40AF"
        
        # DISPLAY CARD
        st.markdown(f"""
        <div class="ticket-card no-print" style='background-color: {bg_col}; color: {txt_col}; border: 10px solid {border};'>
...
# --- MODULE B: DISPLAY (Fixed KeyError) ---
def render_display():
    ...
    with col_q:
        ...
        for t in serving:
            ...
            extra_style = "border: 6px solid gold;" if t.get("appt_name") else ""
            ref_badge = f"<div style='background:white; color:black; font-size:14px; padding:5px; border-radius:5px; margin-top:5px;'>FROM: {t.get('ref_from')}</div>" if t.get('ref_from') else ""

            st.markdown(f"""
            <div style='background-color: {b_col}; color: white; padding: 25px; margin-bottom: 20px; border-radius: 20px; display: flex; justify-content: space-between; {extra_style}; box-shadow: 0 5px 15px rgba(0,0,0,0.2);'>
                <div><div style='font-size: 70px; font-weight: 900;'>{display_name}</div>{ref_badge}</div>
                <div style='text-align: right;'>
                    <div style='font-size: 35px; font-weight:bold;'>{t.get('served_by','Counter')}</div>
                    <div style='font-size: 20px;'>{t['service']}</div>
                </div>
    ...
    with col_v:
        ...
        if parked:
            ...
            st.markdown(f"""
            <div style='background-color: #b91c1c; color: white; padding: 25px; border-radius: 15px; margin-top: 20px; text-align: center; animation: pulse 2s infinite; border: 4px solid white;'>
                <h2 style='margin:0; font-size: 40px;'>⚠ RECALL: {nums}</h2>
                <p style="font-size: 20px;">Please approach counter immediately. Forfeiture in 30 mins.</p>
            </div>""", unsafe_allow_html=True)
    
    # FIXED KEYERROR: Using safe get or defaulting
    announcements = db.announcements if hasattr(db, "announcements") else ["Welcome to SSS."]
    txt = " | ".join(announcements)
    st.markdown(f"<div style='background: #FFD700; color: black; padding: 15px; font-weight: bold; position: fixed; bottom: 0; width: 100%; font-size: 24px;'><marquee>{txt}</marquee></div>", unsafe_allow_html=True)
    ...
    with c1:
        if current:
            ...
            if st.session_state['refer_modal']:
                ...
                if b1.button("✅ COMPLETE", key="comp", use_container_width=True): 
                    current["status"] = "COMPLETED"; current["end_time"] = datetime.datetime.now()
                    db.history.append(current); st.rerun()
                if b2.button("🅿️ PARK", key="park", use_container_width=True): 
                    current["status"] = "PARKED"; current["park_timestamp"] = datetime.datetime.now(); st.rerun()
                if b3.button("🔄 REFER", key="ref", use_container_width=True): 
                    st.session_state['refer_modal'] = True; st.rerun()
        else:
            if st.button("🔊 CALL NEXT TICKET", type="primary", use_container_width=True):
                if queue:
...
# --- MODULE D: ADMIN (Persistent Config) ---
    ...
    if user['role'] == "ADMIN":
        ...
        if mode == "User Management":
            ...
            with c1:
                with st.form("add_user"):
                    ...
                    if st.form_submit_button("Save User"):
                        ...
                        st.success("Saved! Changes are live immediately."); st.rerun()
            with c2:
            ...
            if st.button("Save Lane Config"):
                ...
                st.success("Lanes Updated Persistently!")
