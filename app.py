# ==============================================================================
# SSS G-ABAY v23.5 - BRANCH OPERATING SYSTEM (FINAL GOLD)
# "The Best of V23.4 Visuals + V23.5 Enterprise Power"
...
def get_next_ticket(queue, surge_mode, my_station):
    ...
    # 1. Check for Assigned Appointments for ME
    for t in queue:
    ...
    if surge_mode:
        ...
        return queue[0] if not queue[0].get('assigned_to') else None
    ...
    # Return first unassigned ticket
    for t in queue:
...
def render_admin_panel(user):
    ...
    # RESTORED: COUNTERS
        ...
        st.info("Analytics engine active. View Reports tab for IOMS details.")
        ...
        if tn:
            ...
            if t:
                if t['status'] == "PARKED":
                    ...
                    wait_str = calculate_specific_wait_time(t['id'], t['lane']); people_ahead = calculate_people_ahead(t['id'], t['lane']); c1, c2 = st.columns(2); c1.metric("Est. Wait", wait_str); c2.metric("People Ahead", people_ahead)
